...
func middlerwareHandler(handler http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		fmt.Println("before handler middle start")
		handler.ServeHTTP(w, r)
		fmt.Println("middler finised")
	})
}

func enableCourseMiddlerware(handler http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Add("Access-Control-Allow-Origin", "*")
		w.Header().Add("Access-Control-Allow-Method", "POST, GET, OPTIONS, PUT, DELETE")
		w.Header().Add("Access-Control-Allow-Healers", "Accept, Contetn-Type, Content-Leagth,, Authorization, X-")
		handler.ServeHTTP(w, r)
	})
}
...
func main() {
	...
	http.Handle("/course/", enableCourseMiddlerware(courseItemHandler))
	http.Handle("/course", enableCourseMiddlerware(courseListHandler))
